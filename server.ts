...
async function compileVideoFromScenes(taskId: string, scenes: any[], config: any): Promise<string> {
  ...
  try {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      console.log(`[FFmpeg] Compiling scene ${idx} video...`);
      // scale to 720:1280 (vertical 9:16) for stable stitching
      await execPromise(
        `ffmpeg -y -loop 1 -i "${imgPath}" -i "${audPath}" -c:v libx264 -tune stillimage -c:a aac -b:a 128k -pix_fmt yuv420p -vf "scale=720:1280" -shortest "${clipPath}"`
      );
    }
